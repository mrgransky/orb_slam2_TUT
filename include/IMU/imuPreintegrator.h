...
public:
    ...
    inline Eigen::Vector3d getDeltaP() const {return _delta_P;}   // P_k+1 = P_k + V_k*dt + R_k*a_k*dt*dt/2
    inline Eigen::Vector3d getDeltaV() const {return _delta_V;}   // V_k+1 = V_k + R_k*a_k*dt
    inline Eigen::Matrix3d getDeltaR() const {return _delta_R;}  //  R_k+1 = R_k*exp(w_k*dt)     NOTE: Rwc, Rwc'=Rwc*[w_body]x
    

    // jacobian of delta measurements w.r.t bias of gyro/acc
    inline Eigen::Matrix3d getJPBiasg() const {return _J_P_Biasg;}    // position / gyro
    inline Eigen::Matrix3d getJPBiasa() const {return _J_P_Biasa;}    // position / acc

    inline Eigen::Matrix3d getJVBiasg() const {return _J_V_Biasg;}    // velocity / gyro
    inline Eigen::Matrix3d getJVBiasa() const {return _J_V_Biasa;}   // velocity / acc
    
    inline Eigen::Matrix3d getJRBiasg() const {return _J_R_Biasg;} // rotation / gyro
    

    // noise covariance propagation of delta measurements
    // note: the order is rotation-velocity-position here
    inline Matrix9d getCovPVPhi() const {return _cov_P_V_Phi;}

    inline double getDeltaTime() const {return _delta_time;}

    // skew-symmetric matrix
    static Eigen::Matrix3d skew(const Eigen::Vector3d& v) {return SO3::hat( v );}
    
    // exponential map from vec3 to mat3x3 (Rodrigues formula)
    static Eigen::Matrix3d Expmap(const Eigen::Vector3d& v) {return SO3::exp(v).matrix();}
    ...
    static Eigen::Matrix3d JacobianL(const Eigen::Vector3d& w) {return JacobianR(-w);}
    
	// left jacobian inverse
    static Eigen::Matrix3d JacobianLInv(const Eigen::Vector3d& w) {return JacobianRInv(-w);}
    ...
    inline Eigen::Matrix3d normalizeRotationM (const Matrix3d& R)
    {
        Quaterniond qr(R);
