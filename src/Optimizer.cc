...
void Optimizer::LocalBAPRVIDP(	KeyFrame *pCurKF, const list<KeyFrame*> &lLocalKeyFrames, 
				bool* pbStopFlag, Map* pMap, Mat& gw, LocalMapping* pLM)
{
    // Check current KeyFrame in local window
    if(pCurKF != lLocalKeyFrames.back())
        cerr<<"pCurKF != lLocalKeyFrames.back. check \n LocalBAPRVIDP "<<endl;
...
void Optimizer::GlobalBundleAdjustmentNavStatePRV(	Map* pMap, const Mat& gw, int nIterations, 
...
void Optimizer::LocalBundleAdjustmentNavStatePRV(KeyFrame *pCurKF, const std::list<KeyFrame*> &lLocalKeyFrames, bool* pbStopFlag, Map* pMap, cv::Mat& gw, LocalMapping* pLM)
{
    // Check current KeyFrame in local window
    if(pCurKF != lLocalKeyFrames.back())
        cerr<<"pCurKF != lLocalKeyFrames.back. check\n LocalBundleAdjustmentNavStatePRV\n"<<endl;
...
//	g2o::OptimizationAlgorithmGaussNewton* solver = new g2o::OptimizationAlgorithmGaussNewton(solver_ptr);
	...
	// in new G2O, SparseBlockMatrixXd does not exist, replaced with SparseBlockMatrixX
...
int Optimizer::PoseOptimization(Frame *pFrame, KeyFrame* pLastKF, 
				const IMUPreintegrator& imupreint, const cv::Mat& gw, 
...
void Optimizer::LocalBundleAdjustmentNavState(KeyFrame *pCurKF, const std::list<KeyFrame*> &lLocalKeyFrames, bool* pbStopFlag, Map* pMap, cv::Mat& gw, LocalMapping* pLM)
{
    // Check current KeyFrame in local window
    if(pCurKF != lLocalKeyFrames.back())
        cerr<<"pCurKF != lLocalKeyFrames.back. check \n LocalBundleAdjustmentNavState"<<endl;
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, const list<KeyFrame*> &lLocalKeyFrames, bool* pbStopFlag, Map* pMap, LocalMapping* pLM)
{
    // Check current KeyFrame in local window
    if(pKF != lLocalKeyFrames.back())
        cerr<<"pKF != lLocalKeyFrames.back. check \n LocalBundleAdjustment const list<KeyFrame*> &lLocalKeyFrames"<<endl;
	...
	*/
	...
	for(size_t i=0; i<vpMP.size(); i++)
	{
		...
		for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
		{
			...
			nEdges++;
			const KeyPoint &kpUn = pKF->mvKeysUn[mit->second];
...
// ------------------------------Visual Inerial Added!------------------------------------- //
{
	cout << " Optimizer::local BA V_I version \n called by LocalMapping thread!" << endl;
